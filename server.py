...
class Server():
    def __init__(self,n_clients,n_communications,parameters,n_classes,client_parameters):
        ...
        self.eavesdropper = Eavesdropper(self.train_batch_size,self.n_batch_per_client,self.clients[0].max_len,n_classes = self.n_classes)
    ...
    def train(self):
        ...
        for i in tqdm(range(self.n_communications)):
            if self.successful_round[i] == 1 :
                ...
                evaluations = self.eavesdropper.evaluate(self.obfuscating_parameters)
                self.eavesdropper_accuracy[i] = evaluations[0]
