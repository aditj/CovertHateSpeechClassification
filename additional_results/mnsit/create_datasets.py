...
def create_datasets_clients(N_device = 10, fraction_of_data = 1,batch_size = 40):
    ...
    # have 10% 0 and 90% 1
    df_eav_1 = df[df['label'].apply(lambda x: x not in [0,1,2,3,4,5])].sample(frac=0.1)
    df_eav_0 = df[df['label'].apply(lambda x: x in [0,1,2,3,4,5])].sample(frac=0.9)
