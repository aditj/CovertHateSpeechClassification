...
def policy_from_sigmoid2d(parameters,L,O,E,A,tau):
    ...
    for o in range(O):
        for e in range(E):
            paras = parameters[o*n_thresholds*E+e*n_thresholds:o*n_thresholds*E+e*n_thresholds + n_thresholds]
    ...
    return policy
def plotfromsigmoidpolicy(policy,O,L,E):
    policy_e_0 = policy[::E]
    plt.plot(np.arange(O*L),policy_e_0[:,0])
    plt.savefig('./data/plots/policy_e_0.png')
...
def spsa(initial_parameters,delta,n_iter,T,P,D,lamb,epsilon,rho,L,O,E,A,C_A,C_L,tau):
  ...
  for i in tqdm(range(n_iter)):
   ...
   # print(C_L_plus,C_L_minus)
    ...
    for j in range(pertub.shape[0]):
      if pertub[j]!=0:
        del_C_A[j] = (C_A_plus - C_A_minus)/(delta[i])
        del_C_L[j] = (C_L_plus - C_L_minus)/(delta[i])
    assert del_C_A.shape == parameters.shape

    # if i%100==0:
    #     print(lamb,del_C_A,del_C_L,(C_L_avg-D),C_A_avg)
